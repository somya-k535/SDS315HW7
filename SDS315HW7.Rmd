...
• **The sample proportion of males who folded their left arm on top**: 0.04147465

• **The sample proportion of females who folded their left arm on top**: 0.03225806
...
observed_difference <- prop.test(c(sample_prop_males_left, sample_prop_females_left), c(num_males, num_females))
observed_difference
print(0.004449982-0.003814625)
...
s e ( \^ p 1 − \^ p 2 ) = √ \^ p 1 ⋅ ( 1 − \^ p 1 )/ N 1 + N 1 ⋅ ( 1 − \^ p 2 )/ N 2
...
If we were to take repeated samples of the difference in proportions for males minus females that cross left arm over right arm from people in this data science class, then we would expect that the true difference in proportions to be in between -0.08393973 and 0.1804891 with 95% confidence.
...
prop_voted1996_GOTV = prop.test(
  ...
  n = c(sum(turnout$GOTV_call == 1), sum(turnout$GOTV_call == 1))
)
prop_voted1996_GOTV

prop_AGE_GOTV = prop.test(
  x = c(sum(turnout$AGE == 1 & turnout$GOTV_call == 1), sum(turnout$AGE == 0 & turnout$GOTV_call == 1)),
  n = c(sum(turnout$GOTV_call == 1), sum(turnout$GOTV_call == 1))
)
prop_AGE_GOTV
